...
LoRa newLoRa() {
	...
	new_LoRa.spredingFactor        = SF_7;        // Good speed & range tradeoff
	new_LoRa.bandWidth             = BW_125KHz;   // Required for most networks (e.g., LoRaWAN)
	new_LoRa.crcRate               = CR_4_5;      // Reliable with low overhead
	new_LoRa.power                 = POWER_17db;  // Max for SX1276, ensure antenna & OCP handle it
	new_LoRa.overCurrentProtection = 130;         // 130 mA is typical for PA_BOOST @ 20 dBm
	new_LoRa.preamble              = 8;           // 8 is common; LoRaWAN uses 8 by default

	return new_LoRa;
}

/* ----------------------------------------------------------------------------- *\
		name        : newLoRaLongRange
...
/* ----------------------------------------------------------------------------- *\
		name        : newLoRaLongRangeBoost

		description : PA Boost Long Range (WARNING use good antenna)
		...
		returns     : A LoRa object whith these default values:
										  ...
										  |    spreading factor = 7				       |
											...
											----------------------------------------
\* ----------------------------------------------------------------------------- */
LoRa newLoRaLongRangeBoost() {
	LoRa new_LoRa;

	new_LoRa.frequency              = 915;         // US ISM band
	new_LoRa.spredingFactor         = SF_12;       // Max spreading for best sensitivity/range
	new_LoRa.bandWidth              = BW_125KHz;   // Narrow BW improves sensitivity
	new_LoRa.crcRate                = CR_4_8;      // Most redundancy for reliability
	new_LoRa.power                  = POWER_20db;  // Max power (PA_BOOST required)
	new_LoRa.overCurrentProtection  = 130;         // 130 mA (safe for PA_BOOST mode)
	new_LoRa.preamble               = 12;          // Longer preamble improves detection at range
...
/* -----------------------------------------------------------------------------
 *	name        : LoRa_spi_enable
 *	description	: Some SX127x modules have an enable pin.
